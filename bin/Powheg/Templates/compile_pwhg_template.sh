...
if [ -z "$$CMSSW_BASE" ]; then
    fail_exit "You must run in a CMSSW environment. Exiciting."
else
    pushd $$CMSSW_BASE
fi
...
if [ -e ${rootfolder}/${folderName}/JHUGen.input ]; then
    cp -p ${rootfolder}/${folderName}/JHUGen.input .
fi

if [ -e ${rootfolder}/${folderName}/obj-gfortran/proclib ]; then
    mkdir ./obj-gfortran
    cp -pr ${rootfolder}/${folderName}/*.so ${rootfolder}/${folderName}/proclib obj-gfortran
fi 
